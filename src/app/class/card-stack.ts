...
export class CardStack extends TabletopObject {
  ...
  @SyncVar() raijincount: number = 0;
  @SyncVar() owner: string = '';
  @SyncVar() KAI_flag: boolean = false;
  @SyncVar() KAI: string = "無し";
  @SyncVar() BAN_flag: boolean = false;
  @SyncVar() komaname: string = null;
  @SyncVar() Player: string = null;
  ...
  private setSamePositionFor_rot(card: Card) {
    card.location.name = this.location.name;
    card.location.x = this.location.x;
    card.location.y = this.location.y;
    card.posZ = this.posZ;
