...
export class PeerContext implements IPeerContext {
  ...
  private parse(peerId: string) {
    try {
      ...
      if (isRoom) {
        ...
        this.capacity = regArray.input.slice(regArray[0].length + 1);
        return;
      }
    } catch (e) {
      console.warn(e);
  ...
  static create(userId: string, roomId: string, roomName: string, password: string, capacity: string): PeerContext
  ...
  private static _createRoom(userId: string = '', roomId: string = '', roomName: string = '', password: string = '', capacity: string = ''): PeerContext {
    let digestUserId = this.generateId('******');
    let digestPassword = calcDigestPassword(roomId, password);
    let peerId = `${digestUserId}${roomId}${lzbase62.compress(roomName)}-${digestPassword}-${capacity}`;

    let peerContext = new PeerContext(peerId);
    peerContext.userId = userId;
    peerContext.password = password;
    peerContext.capacity = capacity;
    return peerContext;
  }
