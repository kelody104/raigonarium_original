...
export class LobbyComponent implements OnInit, OnDestroy {
  ...
  async connect(peerContexts: PeerContext[]) {
    await this.tabletopActionService.deleteAllCards(true);
    await this.tabletopActionService.deleteAllCardStacks();
    await this.tabletopActionService.deleteAllCharacters();
    await this.tabletopActionService.deleteAllDiceSymbols(true);
    ...
    Network.open(userId, context.roomId, context.roomName, password, capacity);
      ...
      .on('OPEN_NETWORK', event => {
        ...
        EventSystem.register(triedPeer)
          .on('CONNECT_PEER', event => {
            ...
            SoundEffect.play(PresetSound.enter);
          })
