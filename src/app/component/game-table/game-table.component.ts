...
export class GameTableComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private viewPositionX: number = DefaultViewPosition.viewPositionX;
  private viewPositionY: number = DefaultViewPosition.viewPositionY;
  private viewPositionZ: number = DefaultViewPosition.viewPositionZ;

  private viewRotateX: number = DefaultViewPosition.viewRotateX;
  private viewRotateY: number = DefaultViewPosition.viewRotateY;
  private viewRotateZ: number = DefaultViewPosition.viewRotateZ;
  ...
  onTableTouchTransform(transformX: number, transformY: number, transformZ: number, rotateX: number, rotateY: number, rotateZ: number, event: string, srcEvent: TouchEvent | MouseEvent | PointerEvent) {
    ...
    //
    let scale = (1000 + Math.abs(this.viewPositionZ)) / 1000;
    transformX *= scale;
    transformY *= scale;
  ...
  onContextMenu(e: any) {
    ...
    let submenus = ViewPositionMenu;

    submenus.forEach(item => {
      submenuActions.push({
        name: item.menuName, action: () => {
          this.viewPositionX = item.viewposx; 
          this.viewPositionY = item.viewposy; 
          this.viewPositionZ = item.viewposz;
          this.viewRotateX = item.viewrotx; 
          this.viewRotateY = item.viewroty; 
          this.viewRotateZ = item.viewrotz;
          PeerCursor.myCursor.RotZ = this.viewRotateZ;
          this.gameTable.nativeElement.style.transform = 'translateZ(' + this.viewPositionZ + 'px) translateY(' + this.viewPositionY + 'px) translateX(' + this.viewPositionX + 'px) rotateY(' + this.viewRotateY + 'deg) rotateX(' + this.viewRotateX + 'deg) rotateZ(' + this.viewRotateZ + 'deg) ';
        }
      })
    })
    //menuActions.push({
    //  name: '視点移動', action: null, subActions: submenuActions
    //});
    menuActions.push({
      name: '視点回転（撮影用）', action: () => this.rotation()
    });
    if (Status.Testmode == true) {
      menuActions.push({
        name: '視点確認', action: () => {
          console.log("PosX = " + this.viewPositionX + " PosY = " + this.viewPositionY + " PosZ = " + this.viewPositionZ + " rotX = " + this.viewRotateX + " rotY = " + this.viewRotateY + "rotZ = " + this.viewRotateZ);
        }
      });
    }
 ...
 　async rotation() {
    if (this.RotateFlag == false) this.RotateFlag = true;
    else this.RotateFlag = false;
    for (; this.RotateFlag == true;) {
      this.viewRotateZ--;
