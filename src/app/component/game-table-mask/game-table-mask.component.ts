...
export class GameTableMaskComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  onContextMenu(e: Event) {
    ...
    actions.push({ name: '削除する', action: () => { this.gameTableMask.destroy(); this.gameTableMask = null; SoundEffect.play(PresetSound.sweep); } });

    subActions_Other.push({
      name: 'マップマスク設定を編集', action: () => { this.showDetail(this.gameTableMask); }
    });
