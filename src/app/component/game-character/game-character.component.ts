...
export class GameCharacterComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  onContextMenu(e: Event) {
    ...
    let subActions_Movezone: ContextMenuAction[] = [];

    actions.push({ name: 'チャットパレットを表示', action: () => { this.showChatPalette(this.gameCharacter) } });
    actions.push({ name: `キャラクターの移動`, action: null, subActions: subActions_Movezone });
    actions.push(ContextMenuSeparator);
    actions.push({ name: `その他の操作`, action: null, subActions: subActions_Other });

    subActions_Movezone.push({
      name: '共有インベントリに移動', action: () => {
